...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultsArray =  otuIds.map((otuId, index) => {
      return {
        otu_id: otuId,
    ...
    var top10 = resultsArray.sort(function(a,b) {
    ...
    var barData = [
      {
        y: top10.map(object => "OTU " + object.otu_id).reverse(),
        x: top10.map(object => object.sample_value).reverse(),
        mode: 'markers',
        marker: {size:16},
        text: top10.map(object => "OTU " + object.otu_label).reverse(),
        type: 'bar',
        orientation: 'h'
      }
    ];
    ...
    //otu_ids, sample_values, and otu_labels to the x, y, and text properties, respectively.
